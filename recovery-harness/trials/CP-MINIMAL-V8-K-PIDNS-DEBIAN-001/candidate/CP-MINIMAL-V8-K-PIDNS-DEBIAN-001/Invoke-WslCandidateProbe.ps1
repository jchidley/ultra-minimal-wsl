...
param(
    ...
    [string]$Distribution = 'Debian-Minimal',
    [string]$ExpectedWslVersion = '2.7.12.0',
    [string]$RootfsPath = 'C:\controlled-inputs\ultra-minimal-wsl\debian-13.5-amd64-wsl-rootfs.tar.gz',
    [ValidatePattern('^[0-9a-fA-F]{64}$')]
...
$SmokeCommand = '/bin/true && test -r /proc/self/status && test -d /sys && test -c /dev/null && printf debian-ok'
$SuccessMarker = 'debian-ok'
...
if ($SelfTest) {
    $quoted = Join-WindowsCommandLine @('--distribution', 'Debian-Minimal', '--exec', '/bin/sh', '-c', $SmokeCommand)
...
if ((Get-Sha256 $RootfsPath) -ne $ExpectedRootfsSha256.ToLowerInvariant()) { throw 'Debian rootfs SHA-256 mismatch.' }
...
try {
    ...
    $results.smoke = Invoke-BoundedProcess 'debian-smoke' @('--distribution',$Distribution,'--exec','/bin/sh','-c',$SmokeCommand) $TimeoutSeconds $EvidenceRoot
}
catch {
    $primaryFailure = $_
    ...
    $smokeReachedB6D = $smoke -and -not $smoke.timedOut -and $smoke.streamsCompleted -and $smoke.exitCode -eq 0 -and ([string]$smoke.stdout).Contains($SuccessMarker)
    $eventCaptureFailed = Test-Path -LiteralPath (Join-Path $EvidenceRoot 'windows-events.json.error.json')
    $observedGate = if ($smokeReachedB6D) { 'B6-D' } else { 'UNRESOLVED-TRACE-REQUIRED' }
    ...
    $disposition = if ($infrastructureFailure) { 'infrastructure-failure' } elseif ($classification -eq 'B6-D') { 'candidate-pass' } else { 'candidate-evidence-requires-analysis' }
    ...
    $passed = $classification -eq 'B6-D' -and $managementChecksPassed -and $null -eq $primaryFailure -and $null -eq $traceFailure -and $null -eq $shutdownFailure
    Write-JsonAtomic ([ordered]@{
        schema=1; trialId=$TrialId; candidateId=$CandidateId
