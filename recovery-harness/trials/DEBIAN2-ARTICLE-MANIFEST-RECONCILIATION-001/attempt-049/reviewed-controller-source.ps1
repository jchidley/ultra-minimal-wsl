...
try {
    ...
    $remote = Invoke-Command -Session $session -ScriptBlock {
        ...
        $interactive = (& $wsl --distribution $name --user jack --exec /bin/bash -lic 'set -e; test "$(pi --version)" = 0.84.4; pi list | tr -d "\r" | grep -F "$HOME/git/agent-skills"').Replace("`0", '')
        if ($LASTEXITCODE -ne 0 -or $interactive -notmatch '/home/jack/git/agent-skills') { throw 'Interactive Pi or agent-skills package verification failed.' }
