...
try {
    ...
    $remote = Invoke-Command -Session $session -ScriptBlock {
        ...
        $interactive = (& $wsl --distribution $name --user jack --exec /bin/bash -lic 'set -e; printf "pi="; pi --version; pi list | tr -d "\r" | grep -F "$HOME/git/agent-skills"').Replace("`0", '')
        if ($LASTEXITCODE -ne 0 -or $interactive -notmatch '(?m)^pi=0\.84\.4\s*$' -or $interactive -notmatch '/home/jack/git/agent-skills') { throw 'Interactive Pi or agent-skills package verification failed.' }
        [IO.File]::WriteAllText((Join-Path $evidence 'article-manifest.json'), $manifestText, [Text.UTF8Encoding]::new($false))
        [IO.File]::WriteAllText((Join-Path $evidence 'interactive-pi.log'), $interactive, [Text.UTF8Encoding]::new($false))
        & $wsl --shutdown
    ...
    foreach ($name in @('article-manifest.json','interactive-pi.log')) {
