...
param(
    ...
    [string]$Distribution = 'Toybox-Minimal',
    [string]$ExpectedWslVersion = '2.7.12.0',
    [string]$RootfsPath = 'C:\controlled-inputs\ultra-minimal-wsl\toybox-minimal-wsl-rootfs.tar.gz',
    [ValidatePattern('^[0-9a-fA-F]{64}$')]
    [string]$ExpectedRootfsSha256 = 'a8e81e6d29eac6afa7b9438916df3db00de6f833faf8ab8dfaed482bb9a16fa4',
...
$SmokeCommand = 'test -r /proc/self/status && test -d /sys && test -c /dev/null && printf toybox-ok'
$SuccessMarker = 'toybox-ok'
...
if ($SelfTest) {
    $quoted = Join-WindowsCommandLine @('--distribution', 'Toybox-Minimal', '--exec', '/bin/sh', '-c', $SmokeCommand)
    if ($quoted -notmatch '"test -r /proc/self/status') { throw 'Command-line quoting self-test failed.' }
...
if ((Get-Sha256 $RootfsPath) -ne $ExpectedRootfsSha256.ToLowerInvariant()) { throw 'Toybox rootfs SHA-256 mismatch.' }
...
try {
    ...
    $results.smoke = Invoke-BoundedProcess 'toybox-smoke' @('--distribution',$Distribution,'--exec','/bin/sh','-c',$SmokeCommand) $TimeoutSeconds $EvidenceRoot
}
catch {
    $primaryFailure = $_
    ...
    $smokeReachedB6T = $smoke -and -not $smoke.timedOut -and $smoke.streamsCompleted -and $smoke.exitCode -eq 0 -and ([string]$smoke.stdout).Contains($SuccessMarker)
    $eventCaptureFailed = Test-Path -LiteralPath (Join-Path $EvidenceRoot 'windows-events.json.error.json')
    $observedGate = if ($smokeReachedB6T) { 'B6-T' } else { 'UNRESOLVED-TRACE-REQUIRED' }
    ...
    $disposition = if ($infrastructureFailure) { 'infrastructure-failure' } elseif ($classification -eq 'B6-T') { 'candidate-pass' } else { 'candidate-evidence-requires-analysis' }
    $managementChecksPassed = $results.Contains('version') -and $results.Contains('status') -and $results.Contains('list') -and
        -not $results.version.timedOut -and $results.version.exitCode -eq 0 -and ([string]$results.version.stdout).Contains($ExpectedWslVersion) -and
        -not $results.status.timedOut -and $results.status.exitCode -eq 0 -and
        -not $results.list.timedOut -and $results.list.exitCode -eq 0
    $passed = $classification -eq 'B6-T' -and $managementChecksPassed -and $null -eq $primaryFailure -and $null -eq $traceFailure -and $null -eq $shutdownFailure
    Write-JsonAtomic ([ordered]@{
        schema=1; trialId=$TrialId; candidateId=$CandidateId
...
if ((Get-Content -LiteralPath (Join-Path $EvidenceRoot 'result.json') -Raw | ConvertFrom-Json).passed -ne $true) { throw 'Controlled WSL boot probe did not reach B6-T.' }
