...
$process = Start-Process -FilePath (Join-Path $PSHOME 'pwsh.exe') -Verb RunAs -ArgumentList @('-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-EncodedCommand',$encoded) -PassThru
if (-not $process.WaitForExit(60000)) {
    throw 'Secure run creator did not exit within 60 seconds; inspect durable broker state before retrying.'
}
$exitCode = $process.ExitCode
$process.Dispose()
if ($exitCode -ne 0) { throw "Secure run creation failed with $exitCode." }
