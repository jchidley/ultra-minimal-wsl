...
param(
    ...
    [string]$Distribution = 'Arch-Minimal',
    [string]$ExpectedWslVersion = '2.7.12.0',
    [string]$RootfsPath = 'C:\controlled-inputs\ultra-minimal-wsl\archlinux-bootstrap-2026.07.01-x86_64-wsl-rootfs.tar.gz',
    [ValidatePattern('^[0-9a-fA-F]{64}$')]
    [string]$ExpectedRootfsSha256 = 'c3db227b048aa0e509b8a64d8fd6ab644a73c686fa5198a03e59d69627bad176',
...
$SmokeCommand = '/bin/true && test -r /proc/self/status && test -d /sys && test -c /dev/null && printf arch-ok'
$SuccessMarker = 'arch-ok'
...
if ($SelfTest) {
    $quoted = Join-WindowsCommandLine @('--distribution', 'Arch-Minimal', '--exec', '/bin/sh', '-c', $SmokeCommand)
    if ($quoted -notmatch '"/bin/true && test -r /proc/self/status') { throw 'Command-line quoting self-test failed.' }
...
if ((Get-Sha256 $RootfsPath) -ne $ExpectedRootfsSha256.ToLowerInvariant()) { throw 'Arch rootfs SHA-256 mismatch.' }
...
try {
    ...
    $results.smoke = Invoke-BoundedProcess 'arch-smoke' @('--distribution',$Distribution,'--exec','/bin/sh','-c',$SmokeCommand) $TimeoutSeconds $EvidenceRoot
}
catch {
    $primaryFailure = $_
    ...
    $smokeReachedB6Arch = $smoke -and -not $smoke.timedOut -and $smoke.streamsCompleted -and $smoke.exitCode -eq 0 -and ([string]$smoke.stdout).Contains($SuccessMarker)
    $eventCaptureFailed = Test-Path -LiteralPath (Join-Path $EvidenceRoot 'windows-events.json.error.json')
    $observedGate = if ($smokeReachedB6Arch) { 'B6-ARCH' } else { 'UNRESOLVED-TRACE-REQUIRED' }
    ...
    $disposition = if ($infrastructureFailure) { 'infrastructure-failure' } elseif ($classification -eq 'B6-ARCH') { 'candidate-pass' } else { 'candidate-evidence-requires-analysis' }
    ...
    $passed = $classification -eq 'B6-ARCH' -and $managementChecksPassed -and $null -eq $primaryFailure -and $null -eq $traceFailure -and $null -eq $shutdownFailure
    Write-JsonAtomic ([ordered]@{
        schema=1; trialId=$TrialId; candidateId=$CandidateId
...
if ((Get-Content -LiteralPath (Join-Path $EvidenceRoot 'result.json') -Raw | ConvertFrom-Json).passed -ne $true) { throw 'Controlled WSL boot probe did not reach B6-ARCH.' }
