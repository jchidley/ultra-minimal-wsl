...
$manifestSource = Join-Path $project 'control-plane\test-article\debian2-inputs.v2.json'
$bootstrapSource = Join-Path $project 'control-plane\test-article\Bootstrap-Debian2V10.sh'
$manifestHash = '6bdb47c734ba840d17ee410ef72912afcd202e902ea3d55f9141aaa2eab7af2f'
$bootstrapHash = 'ae02bf156aa2fac25c66d85310d23dd22fe0bb91381a75e6e3342e19eae0da71'
...
if ($operationId -notmatch '^debian2-article-preparation-[0-9]{3}$') { throw 'Protected run ID is not a Debian2 preparation operation.' }
...
if ($inputManifest.schema -ne 1 -or $inputManifest.articleId -ne 'debian2-stock-bootstrap-v4' -or $inputManifest.secrets -ne 'none') {
...
$cleanupResult = $null
$outerFailure = $null
$temporaryAdapterAdded = $false
$constructionSwitch = $null
try {
    $adapters = @(Get-VMNetworkAdapter -VM $vm -ErrorAction Stop)
    if ($adapters.Count -ne 0) { throw 'Disposable fixture network boundary changed; expected zero adapters.' }
    $constructionSwitch = Get-VMSwitch -Name 'Default Switch' -ErrorAction Stop
    if ($constructionSwitch.SwitchType -ne [Microsoft.HyperV.PowerShell.VMSwitchType]::Internal) { throw 'Default Switch is not the expected internal Hyper-V switch.' }
    Add-VMNetworkAdapter -VM $vm -Name 'debian2-construction' -SwitchName $constructionSwitch.Name -ErrorAction Stop
    $temporaryAdapterAdded = $true
...
'@
    ...
    Invoke-Command -Session $session -ScriptBlock {
        ...
        $evidence = 'C:\controlled-wsl-trials\DEBIAN2-ARTICLE-001'
        if (Test-Path -LiteralPath $stage) { Remove-Item -LiteralPath $stage -Recurse -Force }
        if (Test-Path -LiteralPath $evidence) { Remove-Item -LiteralPath $evidence -Recurse -Force }
        [IO.Directory]::CreateDirectory($stage) | Out-Null
        [IO.Directory]::CreateDirectory($evidence) | Out-Null
    }
    Copy-Item -ToSession $session -LiteralPath $manifestSource -Destination 'C:\controlled-inputs\ultra-minimal-wsl\debian2\v1\debian2-inputs.v2.json'
    ...
    $remoteResult = Invoke-Command -Session $session -ScriptBlock {
        ...
        if ((Hash (Join-Path $stage 'debian2-inputs.v2.json')) -ne $ManifestHash -or (Hash (Join-Path $stage 'Bootstrap-Debian2.sh')) -ne $BootstrapHash) { throw 'Staged Debian2 procedure identity mismatch.' }
        $manifest = Get-Content -LiteralPath (Join-Path $stage 'debian2-inputs.v2.json') -Raw | ConvertFrom-Json
        ...
        $upIndexes = @(Get-NetAdapter | Where-Object Status -eq 'Up' | Select-Object -ExpandProperty ifIndex)
        $dnsServers = @(Get-DnsClientServerAddress -AddressFamily IPv4 | Where-Object InterfaceIndex -in $upIndexes | ForEach-Object ServerAddresses | Where-Object { $_ -match '^([0-9]{1,3}\.){3}[0-9]{1,3}$' -and $_ -notmatch '^(127\.|0\.0\.0\.0$)' } | Sort-Object -Unique)
        if ($dnsServers.Count -lt 1) { throw 'The Windows fixture has no configured IPv4 DNS server for stock WSL construction.' }
        $constructionDns = $dnsServers[0]
        $bootstrap = Invoke-Captured 'bootstrap' @('--distribution',$name,'--user','root','--exec','/bin/bash','/mnt/c/controlled-inputs/ultra-minimal-wsl/debian2/v1/Bootstrap-Debian2.sh',$constructionDns) 5400
        & $wsl --shutdown
        ...
        [pscustomobject][ordered]@{
            import=$import; bootstrap=$bootstrap; constructionDns=$constructionDns; distribution=$name; defaultUser=$identity
    ...
    Save-Result ([ordered]@{schema=1;operation=$operationId;preflight=$preflight;article=$remoteResult;constructionSwitch=[ordered]@{name=$constructionSwitch.Name;id=$constructionSwitch.Id.Guid;temporaryAdapter='debian2-construction';originalAdapterCount=0};fixtureMutation='Debian2 retained';completedUtc=[DateTime]::UtcNow.ToString('o')})
}
    ...
    if ($session) {
        if ($outerFailure) {
            ...
            Save-Result ([ordered]@{schema=1;operation=$operationId;failure=$outerFailure.Exception.Message;cleanup=$cleanupResult;completedUtc=[DateTime]::UtcNow.ToString('o')})
        }
    ...
    if ($temporaryAdapterAdded) {
        $temporary = @(Get-VMNetworkAdapter -VMName $vmName -Name 'debian2-construction' -ErrorAction Stop)
        if ($temporary.Count -ne 1) { throw 'Temporary fixture network adapter is unavailable for removal.' }
        Remove-VMNetworkAdapter -VMNetworkAdapter $temporary[0] -Confirm:$false -ErrorAction Stop
    }
}
...
$finalAdapter = @(Get-VMNetworkAdapter -VM $final -ErrorAction Stop)
if ($finalAdapter.Count -ne 0) { throw 'Temporary fixture network adapter was not removed.' }
