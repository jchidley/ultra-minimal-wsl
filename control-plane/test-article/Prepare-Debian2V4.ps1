...
'@
    ...
    Save-Result ([ordered]@{schema=1;operation='debian2-article-preparation-032';preflight=$preflight;article=$remoteResult;fixtureMutation='Debian2 retained';completedUtc=[DateTime]::UtcNow.ToString('o')})
}
    ...
    if ($session) {
        if ($outerFailure) {
            ...
            Save-Result ([ordered]@{schema=1;operation='debian2-article-preparation-032';failure=$outerFailure.Exception.Message;cleanup=$cleanupResult;completedUtc=[DateTime]::UtcNow.ToString('o')})
        }
