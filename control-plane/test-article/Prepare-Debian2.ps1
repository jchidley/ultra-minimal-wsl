...
$bootstrapSource = Join-Path $project 'control-plane\test-article\Bootstrap-Debian2.sh'
$manifestHash = '1925fe7507897b9ff5f330deef08b1e1a85e737a77055b82cb0f8a29512e3ed3'
$bootstrapHash = 'e83241ab7dd53dfab02d1272c55bc61cb3a921ff94c9e9466f7ee98cd3d2b903'
...
'@
    ...
    Save-Result ([ordered]@{schema=1;operation='debian2-article-preparation-028';preflight=$preflight;article=$remoteResult;fixtureMutation='Debian2 retained';completedUtc=[DateTime]::UtcNow.ToString('o')})
}
    ...
    if ($session) {
        if ($outerFailure) {
            ...
            Save-Result ([ordered]@{schema=1;operation='debian2-article-preparation-028';failure=$outerFailure.Exception.Message;cleanup=$cleanupResult;completedUtc=[DateTime]::UtcNow.ToString('o')})
        }
