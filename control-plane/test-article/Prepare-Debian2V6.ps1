...
$bootstrapSource = Join-Path $project 'control-plane\test-article\Bootstrap-Debian2V3.sh'
$manifestHash = '1925fe7507897b9ff5f330deef08b1e1a85e737a77055b82cb0f8a29512e3ed3'
$bootstrapHash = '8ac914830d10fbf6f12ad00239a17dd03345e864c045d2501f97b5ac11cf7c92'
...
$originalSwitchId = [Guid]::Empty
$originalSwitchName = ''
$constructionSwitch = $null
try {
    $adapters = @(Get-VMNetworkAdapter -VM $vm -ErrorAction Stop)
    if ($adapters.Count -ne 1) { throw 'Disposable fixture must have exactly one network adapter.' }
    $adapter = $adapters[0]
    $originalSwitchId = $adapter.SwitchId
    $originalSwitchName = [string]$adapter.SwitchName
    ...
    if ($adapter.SwitchId -ne $constructionSwitch.Id) {
        Connect-VMNetworkAdapter -VMNetworkAdapter $adapter -VMSwitch $constructionSwitch -ErrorAction Stop
        $networkChanged = $true
    }
...
'@
    ...
    Save-Result ([ordered]@{schema=1;operation=$operationId;preflight=$preflight;article=$remoteResult;constructionSwitch=[ordered]@{name=$constructionSwitch.Name;id=$constructionSwitch.Id.Guid;originalSwitchName=$originalSwitchName;originalSwitchId=$originalSwitchId.Guid};fixtureMutation='Debian2 retained';completedUtc=[DateTime]::UtcNow.ToString('o')})
}
    ...
    if ($networkChanged) {
        ...
        if ($adapter.Count -ne 1) { throw 'Fixture network adapter count changed before restoration.' }
        if ($originalSwitchId -eq [Guid]::Empty) {
            Disconnect-VMNetworkAdapter -VMNetworkAdapter $adapter[0] -ErrorAction Stop
        } else {
            $originalSwitch = Get-VMSwitch | Where-Object Id -eq $originalSwitchId
            if (@($originalSwitch).Count -ne 1) { throw 'Original fixture switch is unavailable for restoration.' }
            Connect-VMNetworkAdapter -VMNetworkAdapter $adapter[0] -VMSwitch $originalSwitch -ErrorAction Stop
        }
    }
}
...
if ($finalAdapter.Count -ne 1 -or $finalAdapter[0].SwitchId -ne $originalSwitchId) { throw 'Fixture network adapter was not restored to its original switch state.' }
