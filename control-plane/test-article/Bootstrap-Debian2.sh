...
article_id=debian2-stock-bootstrap-v1
home=/home/jack
...
apt-get install -y ca-certificates chezmoi curl direnv dirmngr git gnupg2 jq openssh-client pinentry-curses restic shellcheck sqlite3 sudo tmux unzip xz-utils zoxide
...
install -d -o jack -g jack "$home/.local/share" "$home/.local/bin" "$home/git" "$home/tools" "$home/work"
clone_bundle() {
  local bundle=$1 destination=$2 commit=$3
...
clone_bundle dotfiles.bundle "$home/.local/share/chezmoi" 05710eae9b09e645f7ab63cb211c740f1aa75c8f
clone_bundle agent-skills.bundle "$home/git/agent-skills" 4dd2f4befbf41910967a41cc43b4e18916c8a262
clone_bundle ak.bundle "$home/git/ak" 7c53208f1b48109d8db1b6ed191131d00872c18f
clone_bundle tools.bundle "$home/tools" 42c10893f78ea81d2ad5b57248cbb864b73cc257
...
runuser -u jack -- env HOME="$home" bash -c '
  set -euo pipefail
...
for item in \
  ...
  [[ $(git -C "$path" rev-parse HEAD) == "$expected" ]] || { echo "Commit mismatch: $path" >&2; exit 1; }
done

runuser -u jack -- env HOME="$home" bash -lc '
  set -euo pipefail
  command -v chezmoi
  command -v pi
  test -r "$HOME/.bashrc"
  test -L "$HOME/.pi/agent/skills" || test -d "$HOME/.pi/agent/skills"
  pi --version
' > /tmp/debian2-shell-verification.log
...
jq -n \
  ...
  --arg kernel "$(uname -r)" \
  --arg node "$(runuser -u jack -- env HOME="$home" bash -lc 'node --version')" \
  --arg npm "$(runuser -u jack -- env HOME="$home" bash -lc 'npm --version')" \
  --arg pi "$(runuser -u jack -- env HOME="$home" bash -lc 'pi --version')" \
  --arg fnm "$(runuser -u jack -- env HOME="$home" "$fnm_dir/fnm" --version)" \
  --arg mcfly "$(runuser -u jack -- env HOME="$home" "$home/.local/bin/mcfly" --version)" \
  --argjson packages "$packages" \
  '{schema:1,articleId:$articleId,distribution:$distribution,defaultUser:"jack",createdUtc:$createdUtc,stockKernel:$kernel,node:$node,npm:$npm,pi:$pi,fnm:$fnm,mcfly:$mcfly,packages:$packages,secrets:"none"}' \
