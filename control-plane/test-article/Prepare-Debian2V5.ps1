...
'@
    ...
    Save-Result ([ordered]@{schema=1;operation='debian2-article-preparation-033';preflight=$preflight;article=$remoteResult;constructionSwitch=[ordered]@{name=$constructionSwitch.Name;id=$constructionSwitch.Id.Guid;originalSwitchName=$originalSwitchName;originalSwitchId=$originalSwitchId.Guid};fixtureMutation='Debian2 retained';completedUtc=[DateTime]::UtcNow.ToString('o')})
}
    ...
    if ($session) {
        if ($outerFailure) {
            ...
            Save-Result ([ordered]@{schema=1;operation='debian2-article-preparation-033';failure=$outerFailure.Exception.Message;cleanup=$cleanupResult;completedUtc=[DateTime]::UtcNow.ToString('o')})
        }
