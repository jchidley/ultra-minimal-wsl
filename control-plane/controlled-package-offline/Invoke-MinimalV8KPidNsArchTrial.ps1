...
$TrialId = 'CP-MINIMAL-V8-K-PIDNS-ARCH-001'
...
$CandidateProbe = Join-Path $InputRoot 'procedure\Invoke-WslArchProbe.ps1'
$RecoveryProbe = Join-Path $InputRoot 'procedure\Invoke-WslCandidateProbe.ps1'
$CandidateRootfs = Join-Path $InputRoot 'archlinux-bootstrap-2026.07.01-x86_64-wsl-rootfs.tar.gz'
...
$CandidateProbeSha256 = '7d30d7473c3b849ff2e0539d487f6bf01a3ab25bfb6e7df011e645be8546889d'
$RecoveryProbeSha256 = '424b0d4f26cef5c717a4805b2577feebf6973de4656778d7800474aad1bf4ebb'
$CandidateRootfsSha256 = 'c3db227b048aa0e509b8a64d8fd6ab644a73c686fa5198a03e59d69627bad176'
...
function Invoke-FixedProbe([bool]$Recovery) {
    ...
    $distribution = if ($Recovery) { 'Toybox-Minimal' } else { 'Arch-Minimal' }
...
function Assert-FixedInputs {
    ...
    if ((Get-Item -LiteralPath $CandidateRootfs).Length -ne 214122848) { throw 'Arch rootfs size mismatch.' }
...
$plan = [ordered]@{
    ...
    candidateGate = 'B6-ARCH'; candidateDistribution = 'Arch-Minimal'; candidateSmoke = '/bin/true plus the plan-defined Arch smoke contract'
    recoveryGate = 'B6-T'; recoveryDistribution = 'Toybox-Minimal'; diagnosticDebugConsole = $true
    note = 'One serial Arch compatibility interval with the frozen minimal-v8 plus K-PIDNS-001 package and kernel. Only the pinned Arch rootfs and plan-defined B6-ARCH probe replace candidate-probe inputs; exact Toybox stock recovery remains mandatory.'
}
