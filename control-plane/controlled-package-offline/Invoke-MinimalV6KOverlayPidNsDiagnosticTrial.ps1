...
$TrialId = 'CP-MINIMAL-V6-K-OVERLAY-PIDNS-DIAG-001'
$InputRoot = 'C:\controlled-inputs\ultra-minimal-wsl'
$EvidenceParent = 'C:\controlled-wsl-trials'
$CandidatePackage = Join-Path $InputRoot 'candidate-packages\minimal-v6-excluded-initialize\wsl.msi'
$CandidateManifest = Join-Path $InputRoot 'trial-manifests\minimal-v6-k-overlay-pidns-001.json'
...
$CandidateProductCode = '{9185A489-303F-439F-A17B-DE95FACC71FC}'
...
$CandidatePackageBytes = 174395392
$CandidatePackageSha256 = '1995a9f3b247e246bfdf8a6a3a566a7a20d0846ac70cc6ecc942bff35780a8df'
$CandidateManifestSha256 = 'ca167d2f1efee2b0d542ce5eee106d203948df5085449de5f1f85393cbbf041a'
$CompleteSourceDiffSha256 = '6dc4920fcf3632161137036c126550ee843b1019d268e291ad586211ceaae2c0'
...
function Invoke-FixedProbe([bool]$Recovery) {
    ...
    $candidateId = if ($Recovery) { 'stock-wsl-2.7.12-recovery' } else { 'minimal-v6-k-overlay-pidns-001' }
...
function Assert-FixedInputs {
    ...
    if ($manifest.candidate_id -ne 'minimal-v6-k-overlay-pidns-001' -or
        $manifest.wsl_version -ne $ExpectedVersion -or
        $manifest.control_plane_parent -ne 'minimal-v6-excluded-initialize' -or
...
$plan = [ordered]@{
    ...
    note = 'Fixture-local diagnostic transaction only; debugConsole is the sole runtime delta, and transport, extraction, and ledger finalization remain external prerequisites.'
}
