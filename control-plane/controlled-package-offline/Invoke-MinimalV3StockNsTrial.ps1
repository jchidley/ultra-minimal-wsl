...
$TrialId = 'CP-MINIMAL-V3-STOCK-NS-001'
$InputRoot = 'C:\controlled-inputs\ultra-minimal-wsl'
$EvidenceParent = 'C:\controlled-wsl-trials'
$CandidatePackage = Join-Path $InputRoot 'candidate-packages\minimal-v3-stock-ns\wsl.msi'
$CandidateManifest = Join-Path $InputRoot 'trial-manifests\minimal-v3-stock-ns.json'
...
$CandidateProductCode = '{E79C2ACE-26AB-48DA-BFFD-467C63035CDD}'
...
$CandidatePackageBytes = 174407680
$CandidatePackageSha256 = 'e9c41512246db9afe15a150a83f457ddd8087479225fee63f1e4a32c5ec8fe77'
$CandidateManifestSha256 = '25ab8c17df7df2c2534542000978dc60a8a4735659f797a8ea6f44afad9cc6f0'
$OutputManifestSha256 = 'ab307e0793d9387c49754b3449513ab80d2fdc195c533d2b3191659c2794f6de'
$CompleteSourceDiffSha256 = '97935ccd9541bb221e87cf2dee746cdcff221b5b69d98d3e43b42fddf69576ad'
...
function Invoke-FixedProbe([bool]$Recovery) {
    ...
    $candidateId = if ($Recovery) { 'stock-wsl-2.7.12-recovery' } else { 'minimal-v3-stock-ns' }
...
function Assert-FixedInputs {
    ...
    if ($manifest.candidate_id -ne 'minimal-v3-stock-ns' -or
        $manifest.wsl_version -ne $ExpectedVersion -or
        $manifest.complete_source_diff_sha256 -ne $CompleteSourceDiffSha256 -or
        $manifest.output_manifest_sha256 -ne $OutputManifestSha256 -or
...
function Invoke-RunnerSelfTest {
    $points = @(
        ...
        'msi-install-candidate', 'msi-install-candidate-after', 'after-candidate-install', 'candidate-probe',
        'msi-remove-candidate', 'msi-remove-candidate-after', 'msi-restore-stock', 'msi-restore-stock-after', 'recovery-probe'
    )
    $results = @()
    foreach ($point in $points) {
        $script:TestState = @{ $StockProductCode = $true; $CandidateProductCode = $false }
        ...
        if (-not $script:TestState[$StockProductCode] -or $script:TestState[$CandidateProductCode]) { throw "Self-test point '$point' did not restore stock state." }
        if ($point -notin @('pre-install-shutdown', 'recovery-probe') -and -not $script:Actions.Contains('recovery-probe')) { throw "Self-test point '$point' did not run independent recovery proof." }
