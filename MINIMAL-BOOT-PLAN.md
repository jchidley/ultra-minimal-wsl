...
The experiment target is the WSL Linux configuration, not its outer fixture. Fixture start, command transport, package placement, evidence extraction, and rollback are prerequisites only. Failure before the first probe `wsl.exe` process is infrastructure failure and creates no candidate result. Do not vary the probe or develop generalized fixture tooling in response. Freeze the tested broker and transport while they satisfy the active trial; repair only a reproduced defect that prevents that trial. Two pre-probe failures with the same cause trigger one bounded root-cause repair rather than further retries or wrapper development.
...
Committed `inventory/experiments.sqlite` records candidates, artifacts, operations, dispositions, configs, and terminal trials transactionally. Query and update it only through `tools/experiment.py`; inspect the logical `diff-head` before commit. The former JSON plan is frozen migration history. Stable runtime policy belongs in immutable hash-bound operation templates. Candidate-specific scripts come from versioned, fixed-phase generation: build records create only a controlled build script and build controller, while runtime records created after package identity exists create only a trial runner and runtime controller. The small structured delta binds parent/output identities, rejects incomplete or ambiguous substitution, and emits the exact generated delta for review; derived display diffs and placeholder outputs are not durable artifacts. Ad hoc copy-and-replace reconstruction is prohibited. Prepare candidate lineage, genuinely new file artifacts, reused identities, and role links atomically, with repository hashes and sizes calculated by the CLI. Derive a comparison from a terminal parent with only explicit artifact-role replacements; create a launch retry by reference without respecifying candidate, controller, artifacts, trial intent, or contract fields. Preserve every candidate generation rather than rewriting it; any ABI or source change creates a new candidate and repeats the source-policy, mutation, and two-build reproducibility gates. Before runtime execution, require complete hashes and agent-validate the exact candidate operation and recovery path from SQL. Operations wholly confined to the dedicated disposable fixture are standing-authorized within that pinned, offline, hash-verified, recoverable envelope and require no human review, approval, confirmation, or gate-opening. Physical-host or shared-WSL effects still require fresh explicit approval.
...
- run the tracked commit-readiness workflow with native-command error propagation so a nonzero test, generator, or Git check cannot fall through;
- extract ETW `GuestLog` evidence through the deterministic header-normalizing tracerpt CSV extractor rather than ad hoc column selection;
...
- begin each attended runtime run through the tested ephemeral fixture broker: install and verify its immutable payload under administrator-protected ACLs, snapshot only the exact reviewed workload into a protected allowlist, treat the normal-user queue as hostile, forbid arbitrary host commands/paths/hashes/executables and arbitrary VM/storage identities, keep credentials/results/evidence out of user-writable trees, and force the exact fixture Off on completion, failure, idle expiry, or maximum lifetime;
