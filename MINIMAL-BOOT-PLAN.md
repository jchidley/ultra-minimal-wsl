...
The comparison order is stock WSL 2.7.12 calibration, the finalized `minimal-v3-stock-ns` diagnostic interval, then the evidence-corrected `minimal-v4-stock-ns` and `minimal-v4-mount-ns` siblings, all with the exact registered Toybox rootfs. The accepted stock calibration and the finalized `minimal-v4-stock-ns` candidate both reached `B6-T` with independent stock recovery; the reduced candidate proves corrected-control-plane viability but does not prove its full namespace bundle necessary. Only the mount-only sibling comparison may select the namespace variant and resume kernel ablation.
