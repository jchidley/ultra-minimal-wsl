...
The comparison order is stock WSL 2.7.12 calibration, the finalized `minimal-v3-stock-ns` diagnostic interval, then the evidence-corrected `minimal-v4-stock-ns`, `minimal-v4-mount-ns`, and `minimal-v5-mount-pid-ns` candidates, all with the exact registered Toybox rootfs. The accepted stock calibration and stock-namespace candidate reached `B6-T`; mount-only reached `B3`, mounted the registered ext4 filesystem, then closed its control socket before `CreateInstanceResult`; restoring PID semantics while continuing to omit IPC and UTS restored `B6-T`. Every interval completed independent `B6-T` stock recovery. Retain mount plus PID namespace semantics and omit IPC and UTS.

The first two PID-enabled reduced kernels both finalized at `B2` under that control plane. `K-PIDNS-001` derived from the storage floor; `K-OVERLAY-PIDNS-001` conservatively retained the only reduced sibling previously observed beyond B3. Both exchanged mini-init configuration, then ended the utility VM before dynamic registered-distro attachment completed; the overlay sibling reproduced the same consequential HCS invalid-handle error. Do not rerun either. Follow source ordering into the retained guest `Initialize` path and remove excluded hard-fails before selecting another kernel group.
