...
The comparison order is stock WSL 2.7.12 calibration, the finalized `minimal-v3-stock-ns` diagnostic interval, then the evidence-corrected `minimal-v4-stock-ns` and `minimal-v4-mount-ns` siblings, all with the exact registered Toybox rootfs. The accepted stock calibration and `minimal-v4-stock-ns` reached `B6-T`; mount-only reached `B3`, mounted the registered ext4 filesystem, then closed its control socket before `CreateInstanceResult`. Both intervals completed independent `B6-T` stock recovery. The coherent full namespace bundle therefore remains the passing baseline, while individual IPC, PID, and UTS necessity remains unresolved. The next narrow minimality ablation restores mount plus PID namespaces while continuing to remove IPC and UTS.
