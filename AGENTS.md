...
Stock WSL 2.7.12 calibration passed `B6-T` with independent recovery in `CP-STOCK-2.7.12-003`. `CP-MINIMAL-V3-STOCK-NS-001` is finalized at `B2`: `/init`, VMBus storage, VSOCK, and the host initial-configuration message were observed, then mini-init rejected an excluded initial-configuration field before ext4 mount and Toybox dispatch. Independent stock recovery passed `B6-T`, inventory integrity is `ok`, and the fixture is Off. Do not rerun the byte-identical candidate. The next action is one evidence-selected host initial-configuration source-policy correction followed by the unchanged build, runtime, and recovery loop. Keep the accepted probe, rootfs, timeouts, instrumentation, evidence schema, classification rules, and recovery requirements unchanged.
