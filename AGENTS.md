...
Stock WSL 2.7.12 calibration passed `B6-T` with independent recovery in `CP-STOCK-2.7.12-003`. `CP-MINIMAL-V3-STOCK-NS-001` is finalized at `B2`; do not rerun it. The evidence-selected `minimal-v4-stock-ns` sender correction passed the fixed Toybox probe at `B6-T` in `CP-MINIMAL-V4-STOCK-NS-001`, independent stock recovery also passed `B6-T`, and the fixture returned Off. This proves viability but not namespace necessity. The `minimal-v4-mount-ns` controlled Windows package build has now completed from pinned offline inputs; extracted package identities independently reverified and the fixture returned Off. Its exact hash-bound runner passes all 13 local fault injections, but two fresh-ID fixture-validation elevation launches remained at `launching` without a worker or output. Retry only that unchanged validation controller when the broker can start it, then continue directly through one fixed candidate interval and mandatory stock recovery. Keep the accepted probe, rootfs, timeouts, instrumentation, evidence schema, classification rules, and recovery requirements unchanged.
