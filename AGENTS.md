...
Stock WSL 2.7.12 calibration passed `B6-T` with independent recovery in `CP-STOCK-2.7.12-003`. `CP-MINIMAL-V3-STOCK-NS-001` is finalized at `B2`; do not rerun it. The evidence-selected `minimal-v4-stock-ns` sender correction passed the fixed Toybox probe at `B6-T` in `CP-MINIMAL-V4-STOCK-NS-001`, independent stock recovery also passed `B6-T`, and the fixture returned Off. This proves viability but not namespace necessity. The next action is retrying the already reproducible `minimal-v4-mount-ns` sibling's hash-bound controlled Windows package build when the elevation broker can start its worker; two bounded fresh-ID launches remained at `launching` and produced no build or candidate result. Continue directly into hash-bound runtime and recovery after package identities exist. Keep the accepted probe, rootfs, timeouts, instrumentation, evidence schema, classification rules, and recovery requirements unchanged.
