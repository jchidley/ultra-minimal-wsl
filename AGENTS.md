...
Stock WSL 2.7.12 calibration passed `B6-T` with independent recovery in `CP-STOCK-2.7.12-003`. `CP-MINIMAL-V3-STOCK-NS-001` is finalized at `B2`: `/init`, VMBus storage, VSOCK, and the host initial-configuration message were observed, then mini-init rejected an excluded initial-configuration field before ext4 mount and Toybox dispatch. Independent stock recovery passed `B6-T`; do not rerun the byte-identical candidate. The evidence-selected `minimal-v4` sender correction and two clean offline Linux builds are complete. The next action is retrying its hash-bound Windows package build under a fresh operation ID; two bounded elevation launches never started a worker or produced a build result. Continue directly into runtime and recovery after package identities exist. Keep the accepted probe, rootfs, timeouts, instrumentation, evidence schema, classification rules, and recovery requirements unchanged.
