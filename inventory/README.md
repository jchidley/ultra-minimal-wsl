...
Candidate-specific controlled build scripts, trial runners, and protected controllers are generated by `tools/generate_candidate_scripts.py` from a versioned structured delta under `control-plane/generation/`. The generator binds parent and output hashes, requires each exact substitution once, rejects duplicate, overlapping, missing, unexpected, or unresolved changes, and prints the complete unified delta from the retained parent. Do not copy a prior script and run ordered string replacements.
