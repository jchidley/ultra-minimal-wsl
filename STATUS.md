...
- Retained-path review localized the earliest minimal-v7 blocker before distro-init's first `CreateInstanceResult`: `ConfigInitializeCommon` iterates `LxssStartupCommon`, where the excluded Windows-interop `binfmt_misc` mount fails with ENOENT and throws from `ConfigInitializeEntry`. `InitEntry` catches that failure and exits, closing the inherited channel; only then does root mini-init enter `ErrorExit` and fail its separate children-file cleanup. This selects `minimal-v8-no-binfmt-mount`, which removes only that startup entry. Its two pinned offline LFS-Builder outputs are byte-identical, but no controlled package or runtime result exists yet.
...
- Two fresh protected-broker launches of the unchanged minimal-v8 build controller ended at UAC before durable broker creation. No worker started, no secure run root was created, no fixture mutation occurred, and no executable SQLite operation exists. The two-failure stop rule prohibits another identical launch without resolving that infrastructure boundary.
