...
- Minimal v6 plus `K-OVERLAY-PIDNS-001` also finalized at `B3`. Overlay removed the prior signal-11 signature, but `WslCorePort` closed after `LaunchInit` without `CreateInstanceResult`; ETW captured no guest log records and selected diagnostic capture rather than another Kconfig group.
...
- The minimal-v6 plus `K-PIDNS-001` and plus `K-OVERLAY-PIDNS-001` combinations are finalized at `B3` and must not be rerun unchanged. Overlay changed the post-`LaunchInit` failure from the known absent-overlay SIGSEGV to an otherwise silent guest-channel closure; it has not established `B4` or selected another kernel facility.
