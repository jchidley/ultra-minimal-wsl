...
- The approved disposable VM `ultra-minimal-wsl-dev` is Off after applying Windows offline. Its verified stock WSL package was staged into the active differencing disk, but the stock baseline was not established. PowerShell Direct is the selected and proven guest-control mechanism: the dedicated random 12-character VM-only local account was created, authenticated as an administrator, and followed by a clean guest shutdown plus independent verification that the VM is Off with no host-attached VM disk.
