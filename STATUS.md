...
- The approved disposable VM `ultra-minimal-wsl-dev` is Off after applying Windows offline. Its verified stock WSL package remains staged, but the stock baseline was not established. Attempts one and two blocked in a recursive VM-query wrapper; attempt three exposed unsupported shutdown syntax; attempt four failed before guest readiness. Attempt five then proved explicit-credential PowerShell Direct remained unavailable for ten minutes; in-artifact shutdown did not prove Off, but separately reviewed ordinary `Stop-VM` recovery succeeded. Final evidence proves Off, zero host-attached disks, and only `clean-shell`; no guest session or MSI installation occurred. Earlier account provisioning proved PowerShell Direct once, but current guest `vmicvmsession` availability is now the active runtime blocker.
