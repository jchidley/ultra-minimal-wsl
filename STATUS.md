...
The v4 runner preserves the accepted probe, timeouts, transaction, and independent recovery mechanics with only candidate identities changed. All 13 fault-injection paths pass; fixture validation independently verified staged hashes, stock installed state `5`, candidate state `-1`, no candidate evidence, and final fixture Off without installing or probing. The first runtime controller attempt then stopped before runner execution on retained generic v3 log names; the corrected controller uses trial-scoped log names and is plan-validated. Its elevation launch remained at `launching`, and one fresh-ID retry was canceled before worker start, so no v4 MSI mutation or candidate probe occurred.
